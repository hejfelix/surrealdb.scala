...
import io.github.hejfelix.surrealdb.client.Poc.{expect, test}
...
object Poc extends IOSuite:
  ...
  test("delete") { client =>
    ...
    yield expect.same(Json.arr(), deleteResult.result) and expect.same(expected, createResult.result)
  ...
  test("query") { client =>
    ...
    for
      ...
      queryResult <- client.singleQuery[Person](s"SELECT * FROM ${newTable.name};", Map.empty)
      names = queryResult.result.map(_.name)
      _ <- client.delete(newTable)
    yield expect.same(List("Felix", "Hannibal", "Louise").sorted, names.sorted)
    end for
